...
CREATE TABLE `user_info` (
   ...
   `review_state` int(4) NOT NULL DEFAULT '' COMMENT '审核状态',
...
CREATE TABLE `cert_info` (
   ...
   `cert_content` text NOT NULL DEFAULT '' COMMENT '证件详情',
   `creat_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
   `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
   PRIMARY KEY (`pk_id`),
   UNIQUE KEY (`did`)
...
CREATE TABLE `schema` (
   ...
   `visitInfo` text NOT NULL DEFAULT '' COMMENT '详细访问信息,格式为Json',
   `creat_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
   `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
   PRIMARY KEY (`pk_id`),
